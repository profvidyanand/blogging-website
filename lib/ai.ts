  ...
  const system = `You are an SEO content writer. Respond ONLY with valid JSON matching this schema:
{
  ...
  "metaDescription": string,
  "summary": string,
  "content": string (HTML using h2/h3/p/ul/li; if the category description requests math or special notation, use $inline$ / $$block$$ LaTeX or <math> MathML — every formula MUST be wrapped in delimiters; never output raw \\begin{bmatrix} or other LaTeX commands without $ or $$),
  "faq": [{"question": string, "answer": string}],
  "tags": string[],
  ...
  const user = [
    ...
    "When including math: wrap every formula in $...$ (inline) or $$...$$ (display/block). Matrices must use delimiters, e.g. $$A = \\begin{bmatrix}1 & 2\\\\ 3 & 4\\end{bmatrix}$$ — never leave \\begin{bmatrix}, \\frac, or other LaTeX commands undelimited in plain text.",
